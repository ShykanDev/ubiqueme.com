...
const getIconColor = (type: string) => {
  switch (type) {
    case 'qr_scan': return 'text-green-400 bg-green-500/10 border-green-500/20';
  ...
  <div class="w-full max-w-7xl mx-auto font-google-sans">
    ...
    <div class="space-y-3">
      <div v-for="notif in notifications" :key="notif.id"
        :class="['p-4 rounded-xl border transition-all flex gap-4 items-start', notif.read ? 'bg-[#111324]/50 border-white/5' : 'bg-[#111324] border-white/10 shadow-lg shadow-black/20']">
        <!-- Icon -->
        <div
          :class="['w-10 h-10 rounded-full flex items-center justify-center shrink-0 border', getIconColor(notif.type)]">
          <svg v-if="notif.type === 'qr_scan'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
            viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm14 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" />
          </svg>
          <svg v-else-if="notif.type === 'system'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
            viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <svg v-else-if="notif.type === 'billing'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
            viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="flex-1">
          <div class="flex justify-between items-start mb-1">
            <h4 :class="['text-sm font-semibold', notif.read ? 'text-gray-300' : 'text-white']">{{ notif.title }}</h4>
            <span class="text-xs text-gray-500 whitespace-nowrap ml-2">{{ notif.date }}</span>
          </div>
          <p :class="['text-[13px] leading-relaxed', notif.read ? 'text-gray-500' : 'text-gray-400']">{{ notif.message
            }}</p>
        </div>
